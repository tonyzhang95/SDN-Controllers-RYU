...
class SimpleSwitch13(app_manager.RyuApp):
	...
	def switch_features_handler(self, ev):
		...
		if dpid == 1: # install rules at switch S1
			self.add_layer4_rules(datapath, inet.IPPROTO_ICMP, '10.0.0.1', 10, 1) # for h1 to port 1
			self.add_layer4_rules(datapath, inet.IPPROTO_ICMP, '10.0.0.2', 10, 2) # for h2 to port 2 (s1)s
	...
	# Function to install general rules
	...
	def handle_ip(self, datapath, in_port, pkt):
		...
		if (datapath.id == 4):
			...
			if (in_port == 1):
				out = parser.OFPPacketOut(datapath, ofproto.OFP_NO_BUFFER, ofproto.OFPP_CONTROLLER, actions, pkt.data)
			else:
				out = parser.OFPPacketOut(datapath, ofproto.OFP_NO_BUFFER, ofproto.OFPP_CONTROLLER, actions1, pkt.data)
			datapath.send_msg(out)
